...
export default class UrbanCommand extends Command {
	...
	public async run(msg: CommandMessage, args: { search: string, number: number }): Promise<Message | Message[]> {
		...
		const { body }: { body: any } = await get(`http://api.urbandictionary.com/v0/define?term=${args.search.split(' ').join('+')}`)
		...
		if (body.list.length === 0) {
			...
			if (!body.list[args.number]) {
				args.number = body.list.length - 1;
			}
			const e: RichEmbed = new RichEmbed()
				...
				.setDescription('\u200b');
			const define: string = body.list[args.number].definition.match(/(.|[\r\n]){1,1024}/g);
			for (let i: number = 0; i < define.length; i++) { e.addField(i === 0 ? 'Definition' : '\u200b', define[i]); }
			const example: string = body.list[args.number].example.match(/(.|[\r\n]){1,1024}/g);
			if (example) {
