...
module.exports = class Anime {
  ...
  async run(msg, params = []) {
    ...
    if (params.join(' ').includes('?')) {
      return msg.channel.sendEmbed(
        new this.bot.methods.Embed().setColor(0xffff00)
  ...
  async authcheck(msg, params) {
    if (this.bot.config.ani_expires <= Math.floor(Date.now() / 1000) + 300) {
      ...
      await message.edit({
        embed: new this.bot.methods.Embed().setColor(0x00ff08)
...
Bitte kontaktiere bitte \`${this.bot.config.owner}\`\n\n${response.error.messages[0]}`)
  ...
  async getanswer(msg, response) {
    let count = 1;
    const message = await msg.channel.sendEmbed(
      new this.bot.methods.Embed().setColor(msg.member.color())
  ...
  answer(response, msg, mes) {
    response.description = this.replaceMap(response.description, { '&amp;': '&', '&lt;': '<', '&gt;': '>', '&quot;': '"', '&#039;': "'", '`': '\'', '<br>': '\n', '<br />': '\n' });
    ...
    else embed.addField('Kapitel | Volumes', `${response.total_chapters} | ${response.total_volumes}`, true);
