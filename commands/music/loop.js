...
module.exports = class ShuffleQueueCommand extends Command {
  ...
  async run(msg, args) {
    ...
    if (!queue) return msg.say('There is nothing to be looped in this guild. Change that!');
    if (!args) return msg.say(`Looping is at the moment ${queue.loop ? 'enabled' : 'disabled'}.`);

    if (['+', 'y', 'yes', 'enable', 'on', '1', 'true'].includes(args.split(' ')[0])) {
      if (queue.loop) return msg.say('Looping is already enabled, my friend.');
      queue.loop = true;
      return msg.say('Looping is now enabled!');
    } else if (['-', 'n', 'no', 'disable', 'off', '0', 'false'].includes(args.split(' ')[0])) {
      if (!queue.loop) return msg.say('Looping is already disabled, my friend.');
      queue.loop = false;
