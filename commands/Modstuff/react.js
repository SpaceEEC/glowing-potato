  ...
  async run(msg, params = []) {
    if (!params[0]) {
      return msg.channel.sendEmbed(new this.bot.methods.Embed()
        .setColor(0xffee00)
        .addField('Fehlender Parameter', 'Bitte ein Emoji und mindestens eine Nachrichten ID angeben.')
    ...
    if (!params[1]) {
      return msg.channel.sendEmbed(new this.bot.methods.Embed()
        .setColor(0xffee00)
        .addField('Fehlender Parameter', 'Bitte mindestens eine Nachrichten ID angeben.')
        .setFooter(`${msg.author.username}: ${msg.content}`, this.bot.user.avatarURL)
      );
    }
    ...
    for (const id of msgs) {
      msg.channel.fetchMessage(id).then(reactmsg => {
        reactmsg.react(emoji).catch(() => msg.channel.sendEmbed(new this.bot.methods.Embed()
          .setColor(0xffee00).addField('Fehler', `Ist das Emoji ${emoji} korrekt?`)
          .setFooter(`${msg.author.username}: ${msg.content}`, msg.author.avatarURL))
        );
      }).catch(() => msg.channel.sendEmbed(new this.bot.methods.Embed()
        .setColor(0xffee00).addField('Fehler', `Ist die ID ${id} korrekt?`)
        .setFooter(`${msg.author.username}: ${msg.content}`, msg.author.avatarURL))
        );
    }
    return msg.delete();
